...
//    }];
    ...
    self.userEditItem = self.viewModel.userEditItem;
    if ([self.userEditItem  isEqualToString: @"name"])
    {
        self.title = @"昵称";
        inputTextField.text =self.viewModel.name;
    ...
    else if ([self.userEditItem  isEqualToString: @"sexuality"])
    {
        ...
        if (self.viewModel.birthday == nil)
        {
            ...
            inputTextField.text = self.viewModel.birthday;
        }
    ...
    {
        self.title = @"电话";
        inputTextField.text =  self.viewModel.account;
...
-(void)saveClicked
{
    ...
    {
        if(![sexTextField.text isEqualToString:curUser.sex])
        {
            isChangeSex = YES;
        }
