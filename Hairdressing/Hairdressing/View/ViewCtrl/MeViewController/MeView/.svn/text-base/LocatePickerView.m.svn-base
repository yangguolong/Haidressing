...
//                User *curUser = [UserService getSelf];
//                if (curUser.birthday)
//                {
//                    NSDate *detaildate=[NSDate dateWithTimeIntervalSince1970:(long)curUser.birthday];
//                    [dataPicker setDate:detaildate animated:NO];
//                }
                break;
            }
            case PickerTypeSex:
            {
                [self loadDuty];
                [locatePickerView selectRow:currSexIndex inComponent:0 animated:YES];
                break;
            }
            case PickerTypeTime:
...
-(void)loadDuty
{
    currIndex = 0;
//    User *curUser = [UserService getSelf];
//    dutyArray = [[NSArray alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"duty.plist" ofType:nil]];
//    for (int i=0; i<[dutyArray count]; i++) {
//        NSString * duty_s = [dutyArray objectAtIndex:i];
//        if([duty_s isEqualToString:curUser.duty]) {
//            currIndex = i;
//        }
//    }
//    currSexIndex = 0;
...
//        if([[sexArray objectAtIndex:j] isEqualToString:curUser.sex]) {
//            currSexIndex = j;
//        }
//    }
}
