...
//            label.textColor = self.tabBar.tintColor;
            ...
            //[UIColor colorWithRed:209.0/255.0 green:78.0/255.0 blue:114.0/255.0 alpha:1.0];
...
- (void)setSelectedViewController:(UIViewController *)selectedViewController {
    ...
    DLog(@"selectIndex %zd",self.selectedIndex);
}
...
- (void)setSelectedIndex:(NSUInteger)selectedIndex {
    if (selectedIndex == 2) {
        if (self.wXdelegate&&[self.wXdelegate respondsToSelector:@selector(didSelectIndex:)]) {
            [self.wXdelegate didSelectIndex:selectedIndex];
            return;
        }
    }
    
    UIViewController * currentViewCtrl = [self.backingViewControllers objectAtIndex:_backingSelectedIndex];
    
    UIViewController * selectViewCtrl = [self.backingViewControllers objectAtIndex:selectedIndex];
    if (selectedIndex != _backingSelectedIndex) {
        
        [selectViewCtrl viewWillAppear:YES];
        [currentViewCtrl viewWillDisappear:YES];
    }
...
//                        [imageView setImage:[UIImage imageNamed:NA]]
//                        switch (<#expression#>) {
//                            case <#constant#>:
//                                <#statements#>
//                                break;
