...
@interface YZMCollectionViewCell:UICollectionViewCell
...
@implementation YZMCollectionViewCell
...
@interface YZMTableViewCell : UITableViewCell
...
@implementation YZMTableViewCell
...
+ (instancetype)indexPathWithCol:(NSInteger)col leftOrRight:(NSInteger)leftOrRight leftRow:(NSInteger)leftRow row:(NSInteger)row {
    YZMIndexPath *indexPath = [[self alloc] initWithColumn:col leftOrRight:leftOrRight leftRow:leftRow row:row];
    return indexPath;
}
@end
...
//@property (nonatomic, strong) YZMFilterView *filterView;
...
@implementation YZMDropDownMenu
...
#pragma mark - setter
- (void)setDataSource:(id<YZMDropDownMenuDataSource>)dataSource {
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        [_collectionView registerClass:[YZMCollectionViewCell class] forCellWithReuseIdentifier:@"CollectionCell"];
        ...
        // filterView init
//        _filterView = [[NSBundle mainBundle] loadNibNamed:NSStringFromClass([YZMFilterView class]) owner:nil options:nil].firstObject;
        
        self.autoresizesSubviews = NO;
        _leftTableView.autoresizesSubviews = NO;
        _rightTableView.autoresizesSubviews = NO;
        _collectionView.autoresizesSubviews = NO;
//        _filterView.autoresizesSubviews = NO;
...
//    layer.position = point;
    ...
    leftDot.opacity = 1.0;
    leftDot.frame = CGRectMake(0, 0, 16.5/1.5,10.5/1.5);
    leftDot.fillColor=[UIColor clearColor].CGColor ;
    [leftDot setContents:(__bridge id)[UIImage imageNamed: @"nav_area_n.png"].CGImage];

    leftDot.position = CGPointMake(point.x, point.y + 2 );
...
//    if ([_dataSource respondsToSelector:@selector(displayByFilterViewInColumn:)]) {
//        displayByFilterView = [_dataSource displayByFilterViewInColumn:tapIndex];
//    }
...
//                [self animateFilterView:_filterView show:NO complete:^{
//                   
//                    [self animateIdicator:_indicators[tapIndex] background:_backGroundView collectionView:collectionView title:_titles[tapIndex] forward:YES complecte:^{
//                        _show = YES;
//                    }];
...
//    else if (displayByFilterView) {
//        
//        YZMFilterView *filterView = _filterView;
//        
//        if (tapIndex == _currentSelectedMenudIndex && _show) {
//            [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView filterView:filterView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
//                _currentSelectedMenudIndex = tapIndex;
//                _show = NO;
//            }];
//            
...
//            
//            if (_currentSelectedMenudIndex!=-1) {
//                // 需要隐藏tableview 和 collectionView
//                
//                [self animateLeftTableView:_leftTableView rightTableView:_rightTableView show:NO complete:^{
//                
//                     [self animateIdicator:_indicators[tapIndex] background:_backGroundView filterView:filterView title:_titles[tapIndex] forward:YES complecte:^{
//                         _show = YES;
//                     }];
//                }];
//                
...
//        UITableView *leftTableView = _leftTableView;
        ...
        if (tapIndex == _currentSelectedMenudIndex && _show) {
            ...
            if (_currentSelectedMenudIndex!=-1) {
                // 需要隐藏collectionview 和 filterView
                [self animateCollectionView:_collectionView show:NO complete:^{
...
//    if ([_dataSource respondsToSelector:@selector(displayByFilterViewInColumn:)]) {
//        
//        displayByFilterView = [_dataSource displayByFilterViewInColumn:_currentSelectedMenudIndex];
//    }
...
//    else if (displayByFilterView) {
//        
//        [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView filterView:_filterView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
//            _show = NO;
//        }];
//        
//    }
...
- (void)animateCollectionView:(UICollectionView *)collectionView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        if (collectionView) {
            ...
            collectionViewHeight = ([collectionView numberOfItemsInSection:0] > 14) ? (5 * 38) : (ceil([collectionView numberOfItemsInSection:0]) * 38); //  /2
        }
...
// *动画显示下拉菜单
// */
//- (void)animateFilterView:(YZMFilterView *)filterView show:(BOOL)show complete:(void(^)())complete {
//    
//    if (show) {
//        
//        CGFloat filterViewHeight = 0;
//        
//        if (filterView) {
//            
//            filterView.frame = CGRectMake(_origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, 0);
//            [self.superview addSubview:filterView];
//          
//            filterViewHeight = [YZMFilterView viewHeight];
//        }
//        [filterView layoutIfNeeded];
//        [UIView animateWithDuration:0.3 animations:^{
//              NSLog(@"showing view %@",NSStringFromCGRect(filterView.frame));
//            if (filterView) {
//                filterView.frame = CGRectMake(_origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, filterViewHeight);
//                [filterView layoutIfNeeded];
//            }
//        }];
//          NSLog(@"%@",NSStringFromCGRect(filterView.frame));
//    } else {
//        [filterView layoutIfNeeded];
//        [UIView animateWithDuration:0.3 animations:^{
//             NSLog(@"hiding View %@",NSStringFromCGRect(filterView.frame));
//            if (filterView) {
//                filterView.frame = CGRectMake(_origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, 0);
//                           [filterView layoutIfNeeded];
//            }
//        } completion:^(BOOL finished) {
//              NSLog(@"hide View completion %@",NSStringFromCGRect(filterView.frame));
//            if (filterView) {
...
//- (void)animateIdicator:(CAShapeLayer *)indicator background:(UIView *)background filterView:(YZMFilterView *)filterView title:(CATextLayer *)title forward:(BOOL)forward complecte:(void(^)())complete{
//    
//    [self animateIndicator:indicator Forward:forward complete:^{
//        [self animateTitle:title show:forward complete:^{
//            [self animateBackGroundView:background show:forward complete:^{
//                [self animateFilterView:filterView show:forward complete:^{
//                    
//                }];
//                
//            }];
...
//    UILabel *titleLabel = (UILabel*)[cell viewWithTag:1];
    ...
    if ([self.dataSource respondsToSelector:@selector(menu:titleForRowAtIndexPath:)]) {
        ...
        titleLabel.text = [self.dataSource menu:self titleForRowAtIndexPath:[YZMIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow row:indexPath.row]];
        // 只取宽度
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
        ...
        [self.delegate menu:self didSelectRowAtIndexPath:[YZMIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow row:indexPath.row]];
...
- (void)confiMenuWithSelectRow:(NSInteger)row leftOrRight:(NSInteger)leftOrRight{
    ...
    title.string = [self.dataSource menu:self titleForRowAtIndexPath:[YZMIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow row:row]];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    YZMCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:collectionCell forIndexPath:indexPath];
    
    if ([self.dataSource respondsToSelector:@selector(menu:titleForRowAtIndexPath:)]) {
        cell.textLabel.text = [self.dataSource menu:self titleForRowAtIndexPath:[YZMIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:-1 leftRow:-1 row:indexPath.row]];
    } else {
...
{
    return CGSizeMake((collectionView.frame.size.width-1), 38); // /2
}
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
        ...
        [self.delegate menu:self didSelectRowAtIndexPath:[YZMIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:-1 leftRow:-1 row:indexPath.row]];
    } else {
        //TODO: delegate is nil
    }
}
...
- (void)confiMenuWithSelectRow:(NSInteger)row{
    CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex];
    title.string = [self.dataSource menu:self titleForRowAtIndexPath:[YZMIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:-1 leftRow:-1 row:row]];
