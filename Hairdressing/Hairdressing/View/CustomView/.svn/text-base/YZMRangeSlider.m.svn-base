...
@property (nonatomic, strong) NSNumberFormatter *decimalNumberFormaYZMer; // Used to format values if formattype is YLRangeSliderFormattypeDecimal

@end

static const CGFloat kLabelsFontSize = 12.0f;

@implementation YZMRangeSlider
...
- (void)updateLabelValues {
    ...
    NSNumberFormatter *formaYZMer = (self.numberFormatterOverride != nil) ? self.numberFormatterOverride : self.decimalNumberFormaYZMer;

    self.minLabel.string = [formaYZMer stringFromNumber:@(self.selectedMinimum)];
    self.maxLabel.string = [formaYZMer stringFromNumber:@(self.selectedMaximum)];
}
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(CGRectInset(self.leftHandle.frame, HANDLE_TOUCH_AREA_EXPANSION, HANDLE_TOUCH_AREA_EXPANSION), gesturePressLocation) || CGRectContainsPoint(CGRectInset(self.rightHandle.frame, HANDLE_TOUCH_AREA_EXPANSION, HANDLE_TOUCH_AREA_EXPANSION), gesturePressLocation))
    {
        ...
        if ([self.delegate respondsToSelector:@selector(didStarYZMouchesInRangeSlider:)]){
            [self.delegate didStarYZMouchesInRangeSlider:self];
        }
...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    //no need to refresh the view because it is done as a sideeffect of seYZMing the property
...
-(void)seYZMintColor:(UIColor *)tintColor{
...
- (NSNumberFormatter *)decimalNumberFormaYZMer {
    if (!_decimalNumberFormaYZMer){
        _decimalNumberFormaYZMer = [[NSNumberFormatter alloc] init];
        _decimalNumberFormaYZMer.numberStyle = NSNumberFormatterDecimalStyle;
        _decimalNumberFormaYZMer.maximumFractionDigits = 0;
    }
    return _decimalNumberFormaYZMer;
}
...
-(void)setNumberFormaYZMerOverride:(NSNumberFormatter *)numberFormaYZMerOverride{
    _numberFormatterOverride = numberFormaYZMerOverride;
    [self updateLabelValues];
}
...
-(void)seYZMintColorBetweenHandles:(UIColor *)tintColorBetweenHandles{
